...
def _try_ink(code: str, out_path: Path) -> bool:
    ...
    try:
        ...
        req = Request(url, headers={"User-Agent": "feishu-bot-claude/mermaid"})
...
def default_cache_dir() -> Path:
    """Default cache location: ~/.cache/feishu-bot-claude/mermaid (or env override)."""
    override = os.environ.get("FEISHU_BOT_MERMAID_CACHE")
    if override:
        return Path(override)
    base = os.environ.get("XDG_CACHE_HOME") or os.path.expanduser("~/.cache")
    return Path(base) / "feishu-bot-claude" / "mermaid"
