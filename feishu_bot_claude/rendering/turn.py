...
from feishu_bot_claude.rendering.card import build_card, build_header, build_image, build_markdown, build_note
...
def render_turn_to_card(
    ...
    # Byte-budget cap: the per-element char limit alone can't stop the
    # cumulative body from blowing past Feishu's ~30KB card limit (code
    # 230025). Walk elements, keep a running serialized-byte total, and trim
    # once we approach the cap. CJK text is ~3 bytes/char in UTF-8, so 40
    # elements × 4000 chars could otherwise reach hundreds of KB.
