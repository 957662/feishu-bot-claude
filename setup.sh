...
wait_for_socket() {
    ...
    echo "WARN: socket 10s 内没出现。看日志: $HOME/.feishu-bot-claude/logs/" >&2
    return 1
}
...
action_install() {
    ...
    echo "  feishu-bot-claude shell    # tmux + Claude Code"
...
action_doctor() {
    ...
    if [ -S "$HOME/.feishu-bot-claude/control.sock" ]; then
        ...
        feishu-bot-claude ping || echo "WARN: socket 在但 ping 失败"
    fi
}
